...
/* Lexer */
extern int augl_lex(YYSTYPE* yylval_param,
                    struct info* yylloc_param,
                    yyscan_t yyscanner);
...
static void augl_error(struct info* locp,
                       struct term** term,
                       yyscan_t scanner,
                       const char* s);
...
static struct term* make_module(char* ident,
                                char* autoload,
                                struct term* decls,
                                struct info* locp);

static struct term* make_bind(char* ident,
                              struct term* params,
                              struct term* exp,
                              struct term* decls,
                              struct info* locp);
static struct term* make_bind_rec(char* ident,
                                  struct term* exp,
                                  struct term* decls,
                                  struct info* locp);
static struct term* make_let(char* ident,
                             struct term* params,
                             struct term* exp,
                             struct term* body,
                             struct info* locp);
...
static struct term* make_regexp_term(char* pattern,
                                     int nocase,
                                     struct info* locp);
static struct term* make_rep(struct term* exp,
                             enum quant_tag quant,
                             struct info* locp);

static struct term* make_get_test(struct term* lens,
                                  struct term* arg,
                                  struct info* info);
static struct term* make_put_test(struct term* lens,
                                  struct term* arg,
                                  struct term* cmds,
                                  struct info* info);
static struct term* make_test(struct term* test,
                              struct term* result,
...
static const yytype_uint8 yytranslate[] = {
    ...
    28, 29, 23, 32, 2,  26, 27, 2,  2,  2,  2,  2,  2,  2,  2,  2, 2, 2, 33, 24,
    2,  21, 2,  22, 2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2, 2, 2, 2,  2,
    2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  30, 2,  31, 2,  2, 2, 2, 2,  2,
    2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2, 2, 2, 2,  2,
    2,  2,  2,  34, 25, 35, 2,  2,  2,  2,  2,  2,  2,  2,  2,  2, 2, 2, 2,  2,
    2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2, 2, 2, 2,  2,
    2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2, 2, 2, 2,  2,
    2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2, 2, 2, 2,  2,
    2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2, 2, 2, 2,  2,
...
static const yytype_uint8 yyprhs[] = {
    0,   0,   3,   9,   12,  13,  20,  26,  32,  38,  39,  43,  49,
    51,  53,  61,  63,  67,  69,  73,  75,  77,  81,  83,  87,  89,
    92,  94,  96,  98,  100, 104, 108, 111, 114, 116, 118, 120, 122,
    124, 126, 128, 130, 133, 134, 140, 142, 144, 146, 150, 152, 154,
    156, 158, 162, 167, 171, 176, 177, 180, 185, 187};

/* YYRHS -- A `-1'-separated list of the rules' RHS.  */
static const yytype_int8 yyrhs[] = {
    37, 0,  -1, 9,  6,  21, 38, 39, -1, 10, 5,  -1, -1, 11, 5,  52, 21, 42, 39,
    -1, 12, 5,  21, 42, 39, -1, 17, 40, 21, 42, 39, -1, 17, 40, 21, 41, 39, -1,
    -1, 48, 18, 42, -1, 48, 19, 48, 20, 42, -1, 22, -1, 23, -1, 11, 5,  52, 21,
    42, 13, 42, -1, 43, -1, 43, 24, 44, -1, 44, -1, 44, 25, 45, -1, 45, -1, 57,
    -1, 45, 26, 46, -1, 46, -1, 46, 27, 47, -1, 47, -1, 47, 49, -1, 49, -1, 51,
    -1, 3,  -1, 4,  -1, 28, 42, 29, -1, 30, 42, 31, -1, 28, 29, -1, 48, 50, -1,
    48, -1, 23, -1, 32, -1, 22, -1, 5,  -1, 7,  -1, 18, -1, 19, -1, 53, 52, -1,
    -1, 28, 54, 33, 55, 29, -1, 5,  -1, 18, -1, 19, -1, 56, 8,  55, -1, 56, -1,
    14, -1, 15, -1, 16, -1, 28, 55, 29, -1, 57, 34, 59, 35, -1, 34, 59, 35, -1,
    58, 34, 59, 35, -1, -1, 60, 58, -1, 60, 21, 3,  58, -1, 3,  -1, -1};

/* YYRLINE[YYN] -- source line where rule number YYN was defined.  */
static const yytype_uint16 yyrline[] = {
    0,   179, 179, 182, 185, 187, 192, 197, 202, 208, 212, 214, 217,
    219, 223, 228, 230, 232, 235, 237, 239, 242, 244, 247, 249, 252,
    254, 257, 259, 261, 263, 265, 267, 270, 272, 275, 277, 279, 282,
    284, 286, 288, 291, 294, 296, 299, 301, 303, 306, 308, 311, 313,
    315, 317, 320, 322, 325, 330, 332, 336, 340, 342};
#endif
...
static const yytype_uint16 yytoknum[] = {
    ...
    267, 268, 269, 270, 271, 272, 273, 274, 275, 61,  63,  42,
    59,  124, 45,  46,  40,  41,  91,  93,  43,  58,  123, 125};
#endif

/* YYR1[YYN] -- Symbol number of symbol that rule YYN derives.  */
static const yytype_uint8 yyr1[] = {
    0,  36, 37, 38, 38, 39, 39, 39, 39, 39, 40, 40, 41, 41, 42, 42,
    43, 43, 44, 44, 44, 45, 45, 46, 46, 47, 47, 48, 48, 48, 48, 48,
    48, 49, 49, 50, 50, 50, 51, 51, 51, 51, 52, 52, 53, 54, 54, 54,
    55, 55, 56, 56, 56, 56, 57, 57, 58, 58, 59, 59, 60, 60};

/* YYR2[YYN] -- Number of symbols composing right hand side of rule YYN.  */
static const yytype_uint8 yyr2[] = {
    0, 2, 5, 2, 0, 6, 5, 5, 5, 0, 3, 5, 1, 1, 7, 1, 3, 1, 3, 1, 1,
    3, 1, 3, 1, 2, 1, 1, 1, 1, 3, 3, 2, 2, 1, 1, 1, 1, 1, 1, 1, 1,
    2, 0, 5, 1, 1, 1, 3, 1, 1, 1, 1, 3, 4, 3, 4, 0, 2, 4, 1, 0};

/* YYDEFACT[STATE-NAME] -- Default rule to reduce with in state
   STATE-NUM when YYTABLE doesn't specify something else to do.  Zero
   means the default is an error.  */
static const yytype_uint8 yydefact[] = {
    0,  0,  0,  0,  1,  4,  0,  9,  3,  0,  0,  0,  2,  43, 0,  28, 29, 38, 39,
    40, 41, 0,  0,  0,  0,  27, 0,  0,  43, 0,  0,  32, 61, 0,  15, 17, 19, 22,
    24, 34, 26, 20, 0,  0,  0,  0,  45, 46, 47, 0,  0,  42, 9,  43, 60, 0,  57,
    30, 0,  0,  0,  0,  25, 37, 35, 36, 33, 61, 31, 12, 13, 9,  9,  10, 0,  0,
    9,  6,  0,  55, 0,  58, 16, 18, 21, 23, 0,  8,  7,  0,  50, 51, 52, 0,  0,
    49, 5,  0,  57, 61, 54, 11, 0,  44, 0,  0,  59, 0,  53, 48, 0,  56, 14};

/* YYDEFGOTO[NTERM-NUM].  */
static const yytype_int8 yydefgoto[] = {-1, 2,  7,  12, 23, 71, 33, 34, 35,
                                        36, 37, 38, 39, 40, 66, 25, 27, 28,
                                        49, 94, 95, 41, 81, 55, 56};

/* YYPACT[STATE-NUM] -- Index in YYTABLE of the portion describing
   STATE-NUM.  */
#define YYPACT_NINF -90
static const yytype_int8 yypact[] = {
    -1,  8,   16,  -4,  -90, 14,  26,  53, -90, 27,  29,  92,  -90, 11,  25,
    -90, -90, -90, -90, -90, -90, 50,  55, 28,  -6,  -90, 1,   30,  11,  55,
    38,  -90, 44,  34,  52,  57,  51,  59, 92,  49,  -90, 64,  56,  22,  55,
    92,  -90, -90, -90, 60,  55,  -90, 53, 11,  -90, 65,  80,  -90, 87,  92,
    92,  92,  -90, -90, -90, -90, -90, 44, -90, -90, -90, 53,  53,  -90, 82,
    -5,  53,  -90, 83,  -90, 100, 73,  57, 51,  59,  92,  74,  -90, -90, 55,
    -90, -90, -90, -5,  79,  104, -90, 55, -90, 44,  -90, -90, 84,  -90, -5,
    101, 73,  81,  -90, -90, 55,  -90, -90};

/* YYPGOTO[NTERM-NUM].  */
static const yytype_int8 yypgoto[] = {
    -90, -90, -90, -34, -90, -90, -22, -90, 61,  66, 58,  62, -9,
    -37, -90, -90, -23, -90, -90, -89, -90, -90, 31, -64, -90};
...
static const yytype_uint8 yytable[] = {
    42,  62, 24, 86,  102, 51,  46, 52,  1,   90,  91, 92,  44, 45,  3,
    109, 4,  5,  77,  47,  48,  72, 73,  93,  6,   15, 16,  17, 76,  18,
    78,  8,  13, 30,  14,  107, 74, 87,  88,  26,  19, 20,  96, 53,  69,
    70,  29, 54, 62,  43,  21,  50, 22,  15,  16,  17, 32,  18, 15,  16,
    17,  30, 18, 57,  9,   10,  30, 101, 19,  20,  11, 63,  64, 19,  20,
    105, 58, 60, 21,  31,  22,  65, 59,  21,  32,  22, 61,  68, 112, 32,
    15,  16, 17, 75,  18,  15,  16, 17,  67,  18,  79, 80,  89, 98,  97,
    19,  20, 99, 103, 100, 19,  20, 104, 108, 110, 21, 111, 22, 84,  82,
    21,  32, 22, 85,  0,   83,  0,  0,   0,   106};

static const yytype_int8 yycheck[] = {
    22, 38, 11, 67,  93, 28, 5,  29, 9,  14, 15, 16, 18, 19, 6,  104, 0,
    21, 52, 18, 19,  43, 44, 28, 10, 3,  4,  5,  50, 7,  53, 5,  5,   11,
    5,  99, 45, 71,  72, 28, 18, 19, 76, 5,  22, 23, 21, 3,  85, 21,  28,
    21, 30, 3,  4,   5,  34, 7,  3,  4,  5,  11, 7,  29, 11, 12, 11,  89,
    18, 19, 17, 22,  23, 18, 19, 97, 24, 26, 28, 29, 30, 32, 25, 28,  34,
    30, 27, 31, 110, 34, 3,  4,  5,  33, 7,  3,  4,  5,  34, 7,  35,  21,
    20, 3,  21, 18,  19, 34, 29, 35, 18, 19, 8,  29, 13, 28, 35, 30,  60,
    58, 28, 34, 30,  61, -1, 59, -1, -1, -1, 98};

/* YYSTOS[STATE-NUM] -- The (internal number of the) accessing
   symbol of state STATE-NUM.  */
static const yytype_uint8 yystos[] = {
    0,  9,  37, 6,  0,  21, 10, 38, 5,  11, 12, 17, 39, 5,  5,  3,  4,  5,  7,
    18, 19, 28, 30, 40, 48, 51, 28, 52, 53, 21, 11, 29, 34, 42, 43, 44, 45, 46,
    47, 48, 49, 57, 42, 21, 18, 19, 5,  18, 19, 54, 21, 52, 42, 5,  3,  59, 60,
    29, 24, 25, 26, 27, 49, 22, 23, 32, 50, 34, 31, 22, 23, 41, 42, 42, 48, 33,
    42, 39, 52, 35, 21, 58, 44, 45, 46, 47, 59, 39, 39, 20, 14, 15, 16, 28, 55,
    56, 39, 21, 3,  34, 35, 42, 55, 29, 8,  42, 58, 59, 29, 55, 13, 35, 42};
...
#define YY_SYMBOL_PRINT(Title, Type, Value, Location)                          \
  do {                                                                         \
    if (yydebug) {                                                             \
      ...
      yy_symbol_print(stderr, Type, Value, Location, term, scanner);           \
      YYFPRINTF(stderr, "\n");                                                 \
    }                                                                          \
  } while (YYID(0))
...
static void yy_symbol_value_print(
    yyoutput, yytype, yyvaluep, yylocationp, term, scanner) FILE* yyoutput;
int yytype;
YYSTYPE const* const yyvaluep;
YYLTYPE const* const yylocationp;
struct term** term;
...
static void yydestruct(
    yymsg, yytype, yyvaluep, yylocationp, term, scanner) const char* yymsg;
int yytype;
YYSTYPE* yyvaluep;
YYLTYPE* yylocationp;
struct term** term;
...
#line 180 "parser.y"
  {
    (*term) = make_module((yyvsp[(2) - (5)].string),
                          (yyvsp[(4) - (5)].string),
                          (yyvsp[(5) - (5)].term),
                          &(yylsp[(1) - (5)]));
  } break;
...
#line 188 "parser.y"
  {
    ...
    (yyval.term) = make_bind((yyvsp[(2) - (6)].string),
                             (yyvsp[(3) - (6)].term),
                             (yyvsp[(5) - (6)].term),
                             (yyvsp[(6) - (6)].term),
                             &(yylsp[(1) - (6)]));
...
#line 193 "parser.y"
  {
    ...
    (yyval.term) = make_bind_rec((yyvsp[(2) - (5)].string),
                                 (yyvsp[(4) - (5)].term),
                                 (yyvsp[(5) - (5)].term),
                                 &(yylsp[(1) - (5)]));
  } break;
...
#line 198 "parser.y"
  {
    LOC_MERGE((yylsp[(1) - (5)]), (yylsp[(1) - (5)]), (yylsp[(4) - (5)]));
    (yyval.term) = make_test((yyvsp[(2) - (5)].term),
                             (yyvsp[(4) - (5)].term),
                             TR_CHECK,
                             (yyvsp[(5) - (5)].term),
                             &(yylsp[(1) - (5)]));
...
#line 213 "parser.y"
  {
    (yyval.term) = make_get_test(
        (yyvsp[(1) - (3)].term), (yyvsp[(3) - (3)].term), &(yyloc));
  } break;
...
#line 215 "parser.y"
  {
    (yyval.term) = make_put_test((yyvsp[(1) - (5)].term),
                                 (yyvsp[(3) - (5)].term),
                                 (yyvsp[(5) - (5)].term),
                                 &(yyloc));
  } break;
...
#line 224 "parser.y"
  {
    ...
    (yyval.term) = make_let((yyvsp[(2) - (7)].string),
                            (yyvsp[(3) - (7)].term),
                            (yyvsp[(5) - (7)].term),
                            (yyvsp[(7) - (7)].term),
                            &(yylsp[(1) - (7)]));
...
#line 231 "parser.y"
  {
    (yyval.term) = make_binop(
        A_COMPOSE, (yyvsp[(1) - (3)].term), (yyvsp[(3) - (3)].term), &(yyloc));
  } break;
...
#line 236 "parser.y"
  {
    (yyval.term) = make_binop(
        A_UNION, (yyvsp[(1) - (3)].term), (yyvsp[(3) - (3)].term), &(yyloc));
  } break;
...
#line 240 "parser.y"
  {
    (yyval.term) =
        make_tree_value((yyvsp[(1) - (1)].tree), &(yylsp[(1) - (1)]));
  } break;
...
#line 243 "parser.y"
  {
    (yyval.term) = make_binop(
        A_MINUS, (yyvsp[(1) - (3)].term), (yyvsp[(3) - (3)].term), &(yyloc));
  } break;
...
#line 248 "parser.y"
  {
    (yyval.term) = make_binop(
        A_CONCAT, (yyvsp[(1) - (3)].term), (yyvsp[(3) - (3)].term), &(yyloc));
  } break;
...
#line 253 "parser.y"
  {
    (yyval.term) = make_binop(
        A_APP, (yyvsp[(1) - (2)].term), (yyvsp[(2) - (2)].term), &(yyloc));
  } break;
...
#line 260 "parser.y"
  {
    (yyval.term) =
        make_string_term((yyvsp[(1) - (1)].string), &(yylsp[(1) - (1)]));
  } break;
...
#line 262 "parser.y"
  {
    (yyval.term) = make_regexp_term((yyvsp[(1) - (1)].regexp).pattern,
                                    (yyvsp[(1) - (1)].regexp).nocase,
                                    &(yylsp[(1) - (1)]));
  } break;
...
#line 271 "parser.y"
  {
    (yyval.term) =
        make_rep((yyvsp[(1) - (2)].term), (yyvsp[(2) - (2)].quant), &(yyloc));
  } break;
...
#line 292 "parser.y"
  {
    ...
    list_cons((yyval.term), (yyvsp[(1) - (2)].term));
  } break;
...
#line 297 "parser.y"
  {
    (yyval.term) = make_param((yyvsp[(2) - (5)].string),
                              (yyvsp[(4) - (5)].type),
                              clone_info(&(yylsp[(1) - (5)])));
  } break;
...
#line 307 "parser.y"
  {
    (yyval.type) =
        make_arrow_type((yyvsp[(1) - (3)].type), (yyvsp[(3) - (3)].type));
  } break;
...
#line 321 "parser.y"
  {
    (yyval.tree) =
        tree_concat((yyvsp[(1) - (4)].tree), (yyvsp[(3) - (4)].tree));
  } break;
...
#line 333 "parser.y"
  {
    (yyval.tree) = make_tree(
        (yyvsp[(1) - (2)].string), NULL, NULL, (yyvsp[(2) - (2)].tree));
  } break;
...
#line 337 "parser.y"
  {
    (yyval.tree) = make_tree((yyvsp[(1) - (4)].string),
                             (yyvsp[(3) - (4)].string),
                             NULL,
                             (yyvsp[(4) - (4)].tree));
  } break;
...
yyerrlab:
  ...
  if (yyerrstatus == 3) {
    ...
    if (yychar <= YYEOF) {
      ...
      yydestruct("Error: discarding", yytoken, &yylval, &yylloc, term, scanner);
      yychar = YYEMPTY;
    }
  }
...
yyerrlab1:
  ...
  for (;;) {
    ...
    yydestruct("Error: popping", yystos[yystate], yyvsp, yylsp, term, scanner);
    YYPOPSTACK(1);
    yystate = *yyssp;
    YY_STACK_PRINT(yyss, yyssp);
  }
...
yyreturn:
  ...
  YY_STACK_PRINT(yyss, yyssp);
  while (yyssp != yyss) {
    yydestruct("Cleanup: popping", yystos[*yyssp], yyvsp, yylsp, term, scanner);
    YYPOPSTACK(1);
  }
#ifndef yyoverflow
  if (yyss != yyssa)
...
static struct term* make_binop(enum term_tag tag,
                               ...
                               struct term* right,
                               struct info* locp) {
  assert(tag == A_COMPOSE || tag == A_CONCAT || tag == A_UNION ||
         tag == A_APP || tag == A_MINUS);
