...
extern char* blkid_devno_to_devname(dev_t devno)
    __ul_attribute__((warn_unused_result));
extern int blkid_devno_to_wholedisk(dev_t dev,
                                    char* diskname,
                                    size_t len,
                                    dev_t* diskdevno)
    __ul_attribute__((warn_unused_result));
...
extern blkid_dev blkid_get_dev(blkid_cache cache,
                               const char* devname,
                               int flags);
...
/* resolve.c */
extern char* blkid_get_tag_value(blkid_cache cache,
                                 const char* tagname,
                                 const char* devname)
    __ul_attribute__((warn_unused_result));
extern char* blkid_get_devname(blkid_cache cache,
                               const char* token,
                               const char* value)
    __ul_attribute__((warn_unused_result));
...
extern int blkid_dev_has_tag(blkid_dev dev,
                             const char* type,
                             const char* value);
...
extern int blkid_parse_tag_string(const char* token,
                                  char** ret_type,
                                  char** ret_val);
...
extern char* blkid_evaluate_tag(const char* token,
                                const char* value,
                                blkid_cache* cache)
    __ul_attribute__((warn_unused_result));
...
/* probe.c */
extern blkid_probe blkid_new_probe(void) __ul_attribute__((warn_unused_result));
extern blkid_probe blkid_new_probe_from_filename(const char* filename)
    __ul_attribute__((warn_unused_result)) __ul_attribute__((nonnull));
...
extern int blkid_probe_set_device(blkid_probe pr,
                                  int fd,
                                  blkid_loff_t off,
                                  blkid_loff_t size)
    __ul_attribute__((nonnull));

extern dev_t blkid_probe_get_devno(blkid_probe pr) __ul_attribute__((nonnull));

extern dev_t blkid_probe_get_wholedisk_devno(blkid_probe pr)
    __ul_attribute__((nonnull));

extern int blkid_probe_is_wholedisk(blkid_probe pr) __ul_attribute__((nonnull));
...
extern int blkid_probe_get_fd(blkid_probe pr) __ul_attribute__((nonnull));

/*
 * superblocks probing
 */
extern int blkid_known_fstype(const char* fstype) __ul_attribute__((nonnull));

extern int blkid_superblocks_get_name(size_t idx,
                                      const char** name,
                                      int* usage);
...
#define BLKID_SUBLKS_LABELRAW                                                  \
  (1 << 2) /* read and define LABEL_RAW result value*/
#define BLKID_SUBLKS_UUID (1 << 3) /* read UUID from superblock */
#define BLKID_SUBLKS_UUIDRAW                                                   \
  (1 << 4) /* read and define UUID_RAW result value */
#define BLKID_SUBLKS_TYPE (1 << 5) /* define TYPE result value */
#define BLKID_SUBLKS_SECTYPE                                                   \
  (1 << 6) /* define compatible fs type (second type) */
...
extern int blkid_probe_filter_superblocks_type(blkid_probe pr,
                                               int flag,
                                               char* names[])
    __ul_attribute__((nonnull));
...
extern int blkid_probe_filter_superblocks_usage(blkid_probe pr,
                                                int flag,
                                                int usage)
    __ul_attribute__((nonnull));
...
extern int blkid_probe_invert_partitions_filter(blkid_probe pr)
    __ul_attribute__((nonnull));
extern int blkid_probe_filter_partitions_type(blkid_probe pr,
                                              int flag,
                                              char* names[])
    __ul_attribute__((nonnull));
...
extern blkid_partition blkid_partlist_get_partition(blkid_partlist ls, int n)
    __ul_attribute__((nonnull));
extern blkid_partition blkid_partlist_get_partition_by_partno(blkid_partlist ls,
                                                              int n)
    __ul_attribute__((nonnull));
extern blkid_partition blkid_partlist_devno_to_partition(blkid_partlist ls,
                                                         dev_t devno)
    __ul_attribute__((nonnull));
 ...
 */
extern int blkid_do_probe(blkid_probe pr) __ul_attribute__((nonnull));
extern int blkid_do_safeprobe(blkid_probe pr) __ul_attribute__((nonnull));
extern int blkid_do_fullprobe(blkid_probe pr) __ul_attribute__((nonnull));

extern int blkid_probe_numof_values(blkid_probe pr) __ul_attribute__((nonnull));
extern int blkid_probe_get_value(blkid_probe pr,
                                 int num,
                                 const char** name,
                                 const char** data,
                                 size_t* len) __ul_attribute__((nonnull(1)));
extern int blkid_probe_lookup_value(blkid_probe pr,
                                    const char* name,
                                    const char** data,
                                    size_t* len)
...
extern int blkid_do_wipe(blkid_probe pr, int dryrun)
    __ul_attribute__((nonnull));
extern int blkid_probe_step_back(blkid_probe pr) __ul_attribute__((nonnull));
